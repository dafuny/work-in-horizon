...
class RenderPipeline(object):
    def __init__(self, **kwargs):
        self.intensity_ambient = convert_type(kwargs.get('intensity_ambient', 0.3))# 阴影强度
        self.intensity_directional = convert_type(kwargs.get('intensity_directional', 0.6))# 指向强度
        self.intensity_specular = convert_type(kwargs.get('intensity_specular', 0.9)) # 高光强度
        self.specular_exp = kwargs.get('specular_exp', 5)
        self.color_ambient = convert_type(kwargs.get('color_ambient', (1, 1, 1))) #阴影颜色
    ...
    def __call__(self, vertices, triangles, color, background):
        height, width = background.shape[:2] # 图片大小
        triangles_new = triangles - 1
        ...
        #     v1, v2, v3 = vertices[[i1-1, i2-1, i3-1], :]
        #     surface_normal = np.cross(v2 - v1, v3 - v1)
        #     normal[[i1-1, i2-1, i3-1], :] += surface_normal
        ...
        # Cython style
        normal = np.zeros((vertices.shape[0], 3), dtype=np.float32)
        mesh_core_cython.get_normal(normal, vertices, triangles_new, vertices.shape[0], triangles.shape[0])
        ...
        # 2. rasterization, [0, 1]
        render_img = render.crender_colors(vertices, triangles_new, color, height, width, BG=background)
