...
def dump_exp_id(param, wfp_id, wfp_exp):
    id, exp = parse_exp_id(param)
    np.savetxt(wfp_id, id, fmt='%.4f')
    np.savetxt(wfp_exp, exp, fmt='%.4f')
...
def draw_landmarks(img, pts, style='fancy', wfp=None, show_flg=False, **kwargs):
    ...
    for i in range(len(pts)):
        if style == 'simple':
            ...
            plt.plot(pts[i][0, :], pts[i][1, :], 'o', markersize=4, color='r')
            index=0
            for xy in zip(pts[i][0, :], pts[i][1, :]):
                plt.annotate("%s" % index, xy=xy, xytext=(np.random.randint(-300,300), np.random.randint(-120,120)), textcoords='offset points',
                             arrowprops=dict(facecolor='green', shrink=0.005, width=0.01, headwidth=0.05))
                index+=1
        elif style == 'fancy':
            alpha = 0.8
            markersize = 4
