...
from utils.inference import get_suffix, parse_roi_box_from_landmark, crop_img, predict_68pts, dump_to_ply, dump_vertex, \
    dump_exp_id, draw_landmarks, predict_dense, parse_roi_box_from_bbox, get_colors, write_obj_with_colors
...
def main(args):
    ...
    # checkpoint_fp = 'training/snapshot/phase1_wpdc_checkpoint_epoch_50.pth.tar'
    arch = 'mobilenet_1'
    ...
    for img_fp in os.listdir(args.files):
        img_ori = cv2.imread(os.path.join(args.files, img_fp))
        print(os.path.join(args.files, img_fp))
        if args.dlib_bbox:
        ...
        for rect in rects:
            ...
            if args.bbox_init == 'two':
                ...
                with torch.no_grad():
                    if args.mode == 'gpu':
                        input = input.cuda()
                    param = model(input)
                    # todo ： maybe for more accurate parameter
                    param = param.squeeze().cpu().numpy().flatten().astype(np.float32)
            ...
            # expression and id
            if args.dump_exp_id:
                wfp_exp = os.path.join(args.files, '{}_exp.txt'.format(img_fp.replace(suffix, '')))
                wfp_id = os.path.join(args.files, '{}_id.txt'.format(img_fp.replace(suffix, '')))
                dump_exp_id(param, wfp_id, wfp_exp)
            ...
            if args.dump_ply:
                dump_to_ply(vertices, tri,
                            os.path.join(args.files, '{}_{}.ply'.format(img_fp.replace(suffix, ''), ind)))
            if args.dump_vertex:
                dump_vertex(vertices, os.path.join(args.files, '{}_{}.mat'.format(img_fp.replace(suffix, ''), ind)))
            if args.dump_pts:
                wfp = '{}_{}.txt'.format(img_fp.replace(suffix, ''), ind)
                wfp = os.path.join(args.files, wfp)
            ...
            if args.dump_paf:
                ...
                wfp_paf = os.path.join(args.files, wfp_paf)
                wfp_crop = '{}_{}_crop.jpg'.format(img_fp.replace(suffix, ''), ind)
                wfp_crop = os.path.join(args.files, wfp_crop)
        ...
        if args.dump_pncc:
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_res:
            draw_landmarks(img_ori, pts_res, wfp=os.path.join(args.files, img_fp.replace(suffix, '_3DDFA.jpg')),
                           show_flg=args.show_flg,style='fancy')
        if args.dump_param:
            wfp = '{}_param.txt'.format(img_fp.replace(suffix, ''))
            wfp = os.path.join(args.files, wfp)
            np.savetxt(wfp, param_res, fmt='%.8f')
            print('Dump to {}'.format(wfp))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dump_exp_id', default='true', type=str2bool)
    parser.add_argument('--dump_param', default='true', type=str2bool)
    parser.add_argument('--files', default='samples/obama', type=str)
